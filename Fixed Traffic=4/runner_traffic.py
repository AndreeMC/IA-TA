...
def run():
    ...
    while Timing <= 3600:
        ...
        if contador == 2:
            for i in range(10):
                halt_average[i]=halt_acum[i]/2
                waiting_average[i]=waiting_acum[i]/2
    ...
    with open('Average Halting Vehicles.csv', mode='w') as csv_file:
        ...
        for i in range(len(halt_180)):
            writer.writerow({'Total Halting vehicles': halt_180[i], 'Total Cycle Time':"900", 'Total Waiting Time': waiting_180[i]})
    # Fin
