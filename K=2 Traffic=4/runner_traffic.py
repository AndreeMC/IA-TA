	...
	while RefreshingTime <= 3600:
		...
		if RefreshingTime < timing:                         # Si el tiempo de actualizacion esta mas proximo que todos los tiempos de la matriz T
			...
			if algorithm == 'ga':
				...
				population = pso.init_population(50, 10, 5, 60)

				best_times, best_fitness = pso.pso_algorithm(population, pso.pso_maximization, totalVeh_average, 0.5, 0.2, 5, 2.5, 500)

				for i in range(len(Phase)):
					CycleTime[i] = Tw
					for j in range(len(Phase[i])):
						NewPhase[i][j] = best_times[i][j]-1
						CycleTime[i] = CycleTime[i] + NewPhase[i][j] + 1
...
def get_options():
	...
	optParser.add_option("--pso", action="store_true", default=False, help="run the commandline version of sumo")
...
if __name__ == "__main__":
	...
	# Set the default algorithm as genetic.
	algorithm = 'ga'
	if options.pso:
		algorithm = 'pso'
